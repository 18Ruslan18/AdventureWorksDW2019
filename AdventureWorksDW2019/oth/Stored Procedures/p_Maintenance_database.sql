...
exec [oth].[p_Maintenance_database] @IndexTimeSecond = 1000, @StatisticsTimeSecond= 1000, @LastPartCountUpdate  = 5
----- 11 min----
*/
CREATE procedure [oth].[p_Maintenance_database] 
	...
	begin try
		...
		while (@count<=@end_cycle and getdate()<@IndexTimeEnd)
		    begin
				...
				insert into [oth].[t_sup_log_Mainteanance]
					(
					 [date_time]  
					,[table_name]
					,[object_name]
		...
		where 
			  case 
					when ([part_number] <> 1 )
						then iif([type_inc] = 1,1,0)
					else 1
				end = 1
		order by [last_update_stat]
